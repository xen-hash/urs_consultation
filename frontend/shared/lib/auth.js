 ...
 *
 * A path on this origin rather than one of the other apps', and it can be:
 * sessionStorage is per-origin, so a session for a role only ever exists in
 * that role's own app. If this code is running and `role` is "teacher", it is
 * running in the faculty app, whose sign-in is its root.
 *
 * Only the student app has a sign-in somewhere other than "/", because it is
 * also the public one and its root is the front page.
 */
export function loginPathFor(role) {
  return role === "student" ? "/sign-in" : "/";
}
