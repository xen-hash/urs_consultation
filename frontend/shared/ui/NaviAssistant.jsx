...
import { PROJECT_CONTACT } from "../lib/constants.js";
import {
...
export default function NaviAssistant() {
  ...
  const answer = useCallback((question) => {
    ...
    const trip = navigationFor(text, { role: currentRole() });
    if (trip) {
      lastActivity.current = Date.now();
      setTurns(prev => trimTurns([...prev, { id, question: text, trip, live: null, answers: [] }]));
      setDraft("");
      setHeard("");
      // A screen on one of the other two deployments is a different origin,
      // which the router cannot reach — asking the faculty app for the student
      // sign-in has to be a real navigation. `assign` rather than `replace`, so
      // the back button still comes home.
      if (isExternal(trip.to)) {
        window.location.assign(trip.to);
      } else {
    ...
    // Which app the reader is in settles which audience a question belongs to
    // far better than its wording does — see audienceForApp in navi-faq.js.
