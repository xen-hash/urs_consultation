...
import { audienceForApp } from "./navi-faq.js";
 ...
 *
 * `app` is which of the three deployments the screen is on, and `to` is its
 * path inside that app. The two are kept apart rather than stored as one
 * address because Navi is in all three apps and answers for all three: a
 * question asked on the faculty portal can name a student screen, and the
 * answer to it has to be a URL on the student origin. resolveDestination()
 * puts them back together, and gives a plain path when the screen is in the
 * app doing the asking.
 *
...
/** The role the reader is acting as: their session first, then the app. */
function readerRole(role, app) {
  if (role) return role;
  const here = audienceForApp(app);
 ...
 */
export function matchDestination(question, { app = THIS_APP, role = null } = {}) {
  ...
  return BY_ID.get(hit.roles[readerRole(role, app)]) || null;
}
 ...
 * to do either way.
 *
 * `to` comes back as a path when the screen is in `from` and as an absolute URL
 * when it is not, which is the only difference the caller has to care about —
 * AppLink and the panel both take either.
 */
export function resolveDestination(dest, role = null, { from = THIS_APP } = {}) {
  if (!dest) return null;
  ...
  if (dest.needs && role !== dest.needs) {
    ...
    return {
      id: gate.id,
      to: urlFor(gate.app, gate.to, { from }),
      label: gate.label,
      text: dest.gate,
    };
  }
  ...
  return {
    id: dest.id,
    to: urlFor(dest.app, dest.hash ? `${dest.to}#${dest.hash}` : dest.to, { from }),
    label: dest.label,
    text: dest.say,
  };
}
...
export function navigationFor(question, { app = THIS_APP, role = null } = {}) {
  return resolveDestination(matchDestination(question, { app, role }), role, { from: app });
}

/**
 * The tab ids a dashboard accepts from the URL, for the dashboards to check.
 *
 * Scoped to one app as well as one path, because all three dashboards now live
 * at /dashboard on their own origin. Without the app, the student dashboard
 * would accept "credentials" — an administration tab it has never had — and
 * quietly try to open it.
 */
export function tabsFor(prefix, app = THIS_APP) {
  return [...new Set(
    DESTINATIONS
      .filter(d => d.hash && d.to === prefix && d.app === app)
      .map(d => d.hash)
  )];
}
