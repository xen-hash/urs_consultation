...
import { currentRole, getSession } from "../lib/auth.js";
import { DAY_LABELS, DAYS } from "../lib/constants.js";
import { urlFor } from "../lib/origins.js";
...
const BOARD = { to: urlFor("student", "/availability"), label: "Open the availability board" };
...
function answerMyRequests(payload) {
  ...
  if (!list.length) {
    return {
      ...
      go: { to: urlFor("student", "/dashboard"), label: "Open your dashboard" },
    };
  }
  ...
  return {
    text: `${lead} ${detail}`,
    go: { to: urlFor("student", "/dashboard#inbox"), label: "Open your inbox" },
  };
}
...
function answerTeacherQueue(payload) {
  ...
  if (!pending.length) {
    return {
      ...
      go: { to: urlFor("faculty", "/dashboard"), label: "Open your dashboard" },
    };
  }
  ...
  return {
    ...
    go: { to: urlFor("faculty", "/dashboard#requests"), label: "Open your requests" },
  };
}
    ...
    if (kind === "my-requests") {
      ...
      if (!student) {
        return {
          ...
          go: { to: urlFor("student", "/sign-in"), label: "Sign in" },
        };
      }
      ...
      if (!teacher) {
        return currentRole() === "student"
          ...
          : {
            ...
            go: { to: urlFor("faculty", "/"), label: "Sign in" },
          };
      }
