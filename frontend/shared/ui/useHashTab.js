 ...
 *
 * So the tab is addressable — /dashboard#inbox — and this hook is what reads
 * it. A hash rather than a route or a query string: it needs no new routes, it
 * survives a refresh, and a link to a tab is a link anybody can send. Which
 * hashes a dashboard answers to is decided by navi-go.js, so the names Navi
 * navigates by and the names the dashboards accept cannot drift apart.
 *
 * All three dashboards are at /dashboard now, each on its own origin, so
 * tabsFor() is asked about this app as well as this path — see navi-go.js.
 *
